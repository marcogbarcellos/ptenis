...
- **ERRATA (Task 1 executada):** o Postgres local roda na porta **5433** (a 5432 estava ocupada). Onde este plano escrever `localhost:5432`, leia/escreva `localhost:5433` — vale para `tests/helpers/db.ts` e `tests/helpers/global-setup.ts` na Task 3.
- **ERRATA (Task 10 executada — botões em forms):** o `Button` do @base-ui renderiza `type="button"` por padrão (suprime o submit nativo!). TODO botão de submit dentro de `<form>` DEVE levar `type="submit"` explícito. Sem isso o form não posta nem com Enter.
- **ERRATA (Task 6 executada — shadcn/@base-ui e fontes):** os componentes shadcn deste projeto são da geração v4 (`base-nova`) sobre **@base-ui/react**, que NÃO suporta `asChild`. Onde este plano usar `asChild` (ex.: `<Button asChild><Link/></Button>`, `<AlertDialogTrigger asChild>`, `<DialogTrigger asChild>`), adapte: prop `render` do @base-ui ou `buttonVariants()` + className direto no elemento. Fontes: as classes de variável da Geist (`geistSans.variable`/`geistMono.variable`) ficam no **`<html>`** (nunca no body — `globals.css` tem `html { @apply font-sans }` e `:root` referencia `--font-geist-sans`; variável CSS não sobe). Atenção também: o `AlertDialogAction` desta geração não fecha o diálogo automaticamente após a action.
- **ERRATA (Task 2 executada — Prisma 7):** (a) a URL do datasource vive em `prisma.config.ts` (lê `DATABASE_URL` do env; `dotenv/config` não sobrescreve env já setado); (b) `PrismaClient` exige driver adapter: `new PrismaClient({ adapter: new PrismaPg({ connectionString }) })` com `@prisma/adapter-pg` — o construtor `datasources: { db: { url } }` da Task 3 NÃO existe mais; (c) `prisma db push --force-reset --skip-generate` está quebrado no Prisma 7 (`--skip-generate` removido; `--force-reset` bloqueado quando invocado por agente de IA). O global-setup da Task 3 deve, em vez disso: conectar no banco de teste com `pg` e rodar `DROP SCHEMA public CASCADE; CREATE SCHEMA public;`, depois `execSync("npx prisma migrate deploy")` com `DATABASE_URL` apontando pro banco de teste; (d) imports de tipos/enums continuam de `@prisma/client`; singleton `db`/`Db`/`Prisma` de `@/lib/db`.
